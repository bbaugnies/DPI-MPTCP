// This file is automatically generated from /home/benjamin/Bro/bro-2.3.1/src/analyzer/protocol/dce-rpc/dce_rpc_simple.pac.
...
#include "/home/benjamin/Bro/bro-2.3.1/build/src/analyzer/protocol/dce-rpc/dce_rpc_simple_pac.h"

namespace binpac {


namespace DCE_RPC_Simple {
ContextDCE_RPC_Simple::ContextDCE_RPC_Simple()
	{
	}
...
int DCE_RPC_PDU::Parse(const_byteptr const t_begin_of_data, const_byteptr const t_end_of_data)
	{
	int t_DCE_RPC_PDU__size;
	...
	// Evaluate 'let' and 'withinput' fields
	t_DCE_RPC_PDU__size = header()->frag_length();
	// Checking out-of-bound for "DCE_RPC_PDU"
	if ( t_begin_of_data + (t_DCE_RPC_PDU__size) > t_end_of_data )
		{
		// Handle out-of-bound condition
		throw binpac::ExceptionOutOfBound("DCE_RPC_PDU",
			(0) + (t_DCE_RPC_PDU__size), 
			(t_end_of_data) - (t_begin_of_data));
		}
	{
	// Setting t_end_of_data with &length
	const_byteptr t_end_of_data = t_begin_of_data + t_DCE_RPC_PDU__size;
	
	// Parse "body"
	body_ = new DCE_RPC_Body(header());
	int t_body__size;
	t_body__size = header()->frag_length() - 16 - header()->auth_length();
	{
	// Setting t_end_of_data with &length
	const_byteptr t_end_of_data = (t_begin_of_data + 16) + t_body__size;
	byteorder_ = header()->byteorder();
	body_->Parse((t_begin_of_data + 16), t_end_of_data, byteorder());
	// Evaluate 'let' and 'withinput' fields
	}
	
	const_byteptr const t_dataptr_after_body = (t_begin_of_data + 16) + (t_body__size);
	BINPAC_ASSERT(t_dataptr_after_body <= t_end_of_data);
	// Parse "auth"
	auth_ = new DCE_RPC_Auth(header());
	int t_auth__size;
	t_auth__size = auth_->Parse(t_dataptr_after_body, t_end_of_data);
	// Evaluate 'let' and 'withinput' fields
	
	// Evaluate 'let' and 'withinput' fields
	}
	BINPAC_ASSERT(t_begin_of_data + (t_DCE_RPC_PDU__size) <= t_end_of_data);
	return t_DCE_RPC_PDU__size;
	}
