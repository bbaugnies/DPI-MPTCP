...
public:
	...
	typedef int (*proc_tcp_option_t)(unsigned int opt, unsigned int optlen,
        ...
        // Callback to process an MPTCP option.
	typedef int (*proc_mptcp_t)(unsigned int optlen,
			const u_char* option, TCP_Analyzer* analyzer,
	...
	static int ParseTCPOptions(const struct tcphdr* tcp,
        ...
        static int ParseMPTCP(const struct tcphdr* tcp,
			proc_mptcp_t proc, TCP_Analyzer* analyzer,
			bool is_orig, void* cookie, TCP_Flags flags);
...
protected:
	...
	static int TCPOptionEvent(unsigned int opt, unsigned int optlen,
				const u_char* option, TCP_Analyzer* analyzer,
				  bool is_orig, void* cookie);
        // No need to pass opt, always 30
	static int MPTCPEvent(unsigned int optlen,
				const u_char* option, TCP_Analyzer* analyzer,
				  bool is_orig, void* cookie, TCP_Flags flags);
